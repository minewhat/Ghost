...
function getBaseUrl(req, slugParam) {
    ...
    if (isTag(req)) {
        baseUrl += '/' + config.routeKeywords.tag + '/' + slugParam + '/rsspost/';
    } else if (isAuthor(req)) {
        baseUrl += '/' + config.routeKeywords.author + '/' + slugParam + '/rsspost/';
    } else {
        baseUrl += '/rss/';
    }
...
generateFeed = function generateFeed(data) {
    ...
    data.results.posts.forEach(function forEach(post) {
        ...
        if(post.tags.length)
            var itemUrl = baseURL + "/collection/" + post.tags[0].slug + "/?id=" + post.url.replace(/\//g, '');
        else
            var itemUrl = baseURL + post.url;

        var htmlContent = processUrls(post.html, data.siteUrl, itemUrl),
            item = {
                title: post.title,
